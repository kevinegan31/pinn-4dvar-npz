'''Pytorch Lightning module for training the NN-Only NPZ model'''
...
class NN(pl.LightningModule):
    def __init__(self, DNN, num_layers, num_neurons,
                 activation_function, learning_rate, dtype):
    ...
    def validation_step(self, batch, batch_idx):
        x, u = batch
        # Constant time-step input retained to match PI-NPZ architecture
...
def forward_nn(model, initial_state, nd_ntot, truth_t):
...
### NN FUNCTIONS -----------------------------------------
# Forward Model NN (Uses PyTorch tensors throughout)
def forward_nn_assimilation(model, nd_initial_state, trajectory_times, dtype):
