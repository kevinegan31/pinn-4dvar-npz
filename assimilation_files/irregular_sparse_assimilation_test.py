...
dt_hours = dt * 24  # step size in hours
steps_per_hour = int(1 / dt_hours)
hourly_indices = np.arange(0, len(truth), steps_per_hour)
num_total = len(hourly_indices)
n_nobs_per_species = int(0.2 * len(hourly_indices))  # 10% of hourly obs
n_pobs_per_species = int(0.1 * len(hourly_indices))  # 10% of hourly obs
n_zobs_per_species = int(0.1 * len(hourly_indices))  # 10% of hourly obs
# Randomize the fraction slightly
n_fraction = np.clip(np.random.normal(loc=0.20, scale=0.05), 0.05, 0.5)
p_fraction = np.clip(np.random.normal(loc=0.10, scale=0.02), 0.02, 0.3)
z_fraction = np.clip(np.random.normal(loc=0.10, scale=0.02), 0.02, 0.3)

# Compute number of observations to sample
nobs_per_species = int(n_fraction * num_total)
n_pobs_per_species = int(p_fraction * num_total)
n_zobs_per_species = int(z_fraction * num_total)
print(nobs_per_species)
obs_times_N = np.sort(np.random.choice(hourly_indices, size=nobs_per_species, replace=False))
obs_times_P = np.sort(np.random.choice(hourly_indices, size=n_pobs_per_species, replace=False))
obs_times_Z = np.sort(np.random.choice(hourly_indices, size=n_zobs_per_species, replace=False))

n_days = 7

# Confirm
print(f"N obs/day: {len(obs_times_N) / n_days}")
print(f"P obs/day: {len(obs_times_P) / n_days}")
print(f"Z obs/day: {len(obs_times_Z) / n_days}")
...
joblib.dump(xb_dict, f"/share/tempest2/egank31/assimilation_results/irregular_sparse_xb_data_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_frozen_params_{NUM_LAYERS}_{NUM_NEURONS}_compressed.pkl")
...
joblib.dump(rk4_assimilation_results, f"/share/tempest2/egank31/assimilation_results/irregular_sparse_rk4_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_compressed.pkl")
...
joblib.dump(pi_npz_assimilation_results, f"/share/tempest2/egank31/assimilation_results/irregular_sparse_pi_npz_frozen_params_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_{NUM_LAYERS}_{NUM_NEURONS}_compressed.pkl")
...
# joblib.dump(franks_pinn_assimilation_results, f"/share/tempest2/egank31/assimilation_results/irregular_sparse_franks_pinn_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_compressed.pkl")
...
# joblib.dump(nn_assimilation_results, f"/share/tempest2/egank31/assimilation_results/irregular_sparse_reg_nn_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_compressed.pkl")