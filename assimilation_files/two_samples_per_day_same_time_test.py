#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Force single-threaded BLAS/PyTorch inside each worker
...
# def pinn_worker_cached(i, x0_np, nd_ntot, checkpoint_path, num_layers, num_neurons, learning_rate, params_dict_copy):
#     """Worker loads model once per process and caches it"""
    
#     # Set threads only once per worker process
#     if not hasattr(pinn_worker_cached, '_threads_set'):
#         torch.set_num_threads(1)
#         torch.set_num_interop_threads(1)
#         pinn_worker_cached._threads_set = True
    
#     try:
#         # Check if model is already loaded in this worker process
#         if not hasattr(pinn_worker_cached, '_model'):
#             checkpoint = torch.load(checkpoint_path, map_location="cpu")
#             pinn_worker_cached._model = PhysicsInformedNN(
#                 DNN=pi_npz_DNN,
#                 num_layers=num_layers,
#                 num_neurons=num_neurons,
#                 activation_function=nn.GELU,
#                 params_dict=params_dict_copy,
#                 lambda_u=1,
#                 lambda_f=1,
#                 learning_rate=learning_rate,
#                 dtype=torch.float64
#             )
#             pinn_worker_cached._model.load_state_dict(checkpoint["state_dict"])
#             pinn_worker_cached._model = pinn_worker_cached._model.to(dtype=torch.float64, device="cpu")
#             pinn_worker_cached._model.eval()
        
#         # Use cached model
#         x0_tensor = torch.tensor(x0_np / nd_ntot, dtype=torch.float64, device="cpu")
#         start = time.time()
#         pred_tensor, _ = forward_pinn(pinn_worker_cached._model, x0_tensor, truth_t)
#         end = time.time()
#         pred = pred_tensor.detach().cpu().numpy() * nd_ntot
#         if i % 1000 == 0:
#             print(f"Completed worker {i}", flush=True)
#         return i, x0_np, pred, end - start
#     except Exception as e:
#         print(f"Error in worker {i}: {e}", flush=True)
#         raise

# # Convert entire list to array once
# initial_guesses_array = np.array(initial_guesses)
# start_time = time.time()
# print(f"Start time: {datetime.datetime.fromtimestamp(start_time)}")

# pi_npz_xb0_results = Parallel(
#     n_jobs=num_jobs, 
#     backend="loky",
#     batch_size=100
# )(
#     delayed(pinn_worker_cached)(
#         i, x0_np, nd_ntot,
#         pi_npz_checkpoint_path,
#         NUM_LAYERS,
#         NUM_NEURONS,
#         LEARNING_RATE,
#         params_dict
...
#     for i, xb_0, pred, duration in pi_npz_xb0_results
# }
# ### Franks PINN Background Results
# start_time = time.time()
# print(f"Start time: {start_time}")
# franks_pinn_xb0_results = Parallel(n_jobs=num_jobs, backend="loky", batch_size=10)(
#     delayed(pinn_worker)(i, x0_np, nd_ntot, franks_pinn_model)
#     for i, x0_np in enumerate(initial_guesses_array)
# )
# end_time = time.time()
# pinn_prediction_loop_time = end_time - start_time
# print(f"Running time: {pinn_prediction_loop_time:.2f}")
# print(f"PINN Forward Prediction Complete - {len(franks_pinn_xb0_results)} trajectories computed.")
# # === Rebuild Dictionary ===
# franks_pinn_xb0_dict = {
#     i: {
#         "perturbed_initial_state": xb_0,
#         "background_state": pred,
#         "run_time": duration
...
#     }
#     for i, xb_0, pred, duration in franks_pinn_xb0_results
# }
# ### Regular NN Background Results
# # === Worker Function ===
# def nn_worker(i, x0_np, nd_ntot, model):
#     x0_tensor = torch.tensor(x0_np / nd_ntot, dtype=dtype, device="cpu")
#     start = time.time()
#     # pred, _ = forward_pinn(gelu_model, x0_tensor, truth_t).detach().numpy()
#     pred_tensor, _ = forward_nn(model, x0_tensor, truth_t)
#     pred = pred_tensor.detach().cpu().numpy() * nd_ntot
#     end = time.time()
#     duration = end - start
#     return i, x0_np, pred, duration

# start_time = time.time()
# print(f"Start time: {start_time}")
# reg_nn_xb0_results = Parallel(n_jobs=num_jobs, backend="loky", batch_size=10)(
#     delayed(nn_worker)(i, x0_np, nd_ntot, nn_model)
#     for i, x0_np in enumerate(initial_guesses_array)
# )
# end_time = time.time()
# pinn_prediction_loop_time = end_time - start_time
# print(f"Running time: {pinn_prediction_loop_time:.2f}")
# print(f"NN Forward Prediction Complete - {len(reg_nn_xb0_results)} trajectories computed.")
# # === Rebuild Dictionary ===
# reg_nn_xb0_dict = {
#     i: {
#         "perturbed_initial_state": xb_0,
...
#     }
#     for i, xb_0, pred, duration in reg_nn_xb0_results
# }
...
joblib.dump(pi_npz_assimilation_results, f"/share/tempest2/egank31/assimilation_results/two_samples_per_day_pi_npz_frozen_params_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_{NUM_LAYERS}_{NUM_NEURONS}_compressed.pkl")
...
# joblib.dump(franks_pinn_assimilation_results, f"/share/tempest2/egank31/assimilation_results/two_samples_per_day_franks_pinn_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_compressed.pkl")
...
# joblib.dump(nn_assimilation_results, f"/share/tempest2/egank31/assimilation_results/two_samples_per_day_reg_nn_assimilation_{NUM_XB0}xb_estimates_min_guess_{MIN_GUESS_VAL}_{num_cg_iterations}_compressed.pkl")